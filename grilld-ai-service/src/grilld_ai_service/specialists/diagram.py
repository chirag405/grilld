...
Phase 5 scope note: produces Mermaid *source* only (.mmd files), not the
rendered SVG/PNG from product-and-architecture.md §5's output package.
Rendering needs a Mermaid CLI/headless-browser step this service doesn't
have yet - a packaging-pipeline concern, natural to build alongside Phase
6's packager (zip/Mermaid/S3), not before it exists. Documented as a known
...
DIAGRAM_AGENT = {
    ...
    "system_prompt": """You are Grilld's Diagram Agent. Read ARCHITECTURE.md and INFRA.md \
...
project's architecture diagram should be simple, not padded out to look impressive. Report back \
a 1-2 sentence summary of what you diagrammed.""",
    "tools": [],
}
