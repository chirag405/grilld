...
## Phase 8: a hardening pass, not new features

The kickoff plan reserves Phase 8 as "buffer/hardening pass over 1-7 before frontend starts" with the gate "all Phase 1-7 gates still pass together, no regressions" - deliberately not a task list of its own, since what needs hardening depends on what the first seven phases actually turned out to need. Wiring `requestingUserId` through `GenerationController.generate()` in Phase 7 task 1 made a pre-existing gap impossible not to notice: every other `sessionId`- or `runId`-scoped endpoint across `SessionController`, `RunReportController`, and `PackageController` took the id from the URL and used it directly, with no check that the authenticated caller actually owned it. Any logged-in user who learned another user's session or run id (a UUID, not trivially guessable, but IDs leak through logs, referrers, and shared links) could read their interview answers, watch their Run Report live, or download their finished blueprint package.

Fixed with the smallest change that closes the gap: `SessionService.verifyOwnership(sessionId, userId)` and `GenerationService.resolveOwningUserId(runId)` (walking `runId -> brief -> session -> userId` for the two controllers that only have a run id, not a session id, in scope). Both are called at the top of each controller method, before anything else runs, and throw the same `AccessDeniedException` (403, already handled by `GlobalExceptionHandler`) `GenerationService.generate()` started using in Phase 7.

**Why this isn't threaded through `SessionService`'s existing method signatures instead:** the more "consistent" fix would add a `requestingUserId` parameter to `submitAnswer`, `calibrateScale`, `forceConclude`, and `overrideScaleTier` themselves, matching how `GenerationService.generate()` does its own check inline. Checked first how many call sites that would touch: 12 test files call these methods directly (bypassing HTTP entirely, which is exactly why this gap existed without any test catching it), all built across Phases 2-6. Threading a new required parameter through would mean editing all 12 for a security property that's fully enforceable at the API boundary alone - the tests that call `SessionService` directly are proving interrogation-engine logic, not auth, and don't need to know about it. Kept the fix contained to the three controllers plus one new method each on `SessionService`/`GenerationService`; zero existing tests needed to change, only new ones were added (`SessionFlowIntegrationTest.answerEndpointRejectsAnotherUsersToken`, `RunReportControllerTest.reportEndpointRejectsAnotherUsersToken`, `PackageControllerTest`'s download-as-intruder assertion) to prove the new behavior itself.

**A related gap considered and deliberately left alone:** `credit_transactions` has no row for the free 60-credit signup grant itself - only for every change *after* that (deductions, refunds, purchase grants). Summing a brand-new user's transactions therefore doesn't reconcile to their balance; it's off by exactly 60. Logging that grant would mean `UserService` (package `user`) taking a dependency on `CreditTransactionRepository` (package `billing`), which already depends back on `user` via `UserRepository` - a circular package dependency for a gap that never produces an incorrect balance, only an incomplete *derivation* of one. Left as a documented, intentional read of "never mutate balance without a row": the signup grant is the account's initial state, not a mutation of it. Worth revisiting if a support/admin tool ever needs to reconstruct a balance purely by summing transactions.

## Phase 9 (backend side): what the frontend actually needed from Spring

Phase 9 is `grilld-frontend/` - see that project's own `LEARNING.md` for the frontend-specific narrative (design system, Next.js 16, the BFF proxy architecture). This section covers the small set of backend changes the frontend's first real screen (the interview loop) exposed as missing, using the same "build only what's actually asked for by real usage" discipline as every prior phase.

**`OAuth2LoginSuccessHandler` stopped returning raw JSON.** It was always a placeholder - its own Javadoc said so - since there was no frontend origin to redirect to. Now it redirects to `{grilld.frontend.base-url}/auth/callback?token=...`, a new frontend Route Handler that moves the token into an httpOnly cookie and immediately drops it from the URL. `grilld.frontend.base-url` gets a working `http://localhost:3000` default rather than DB_URL/GOOGLE_CLIENT_ID's fail-fast treatment - it's operational config, not a secret, same reasoning as `grilld.ai-service.base-url`.

**CORS didn't exist until now** because nothing before Phase 9 called the API from a different origin - every test uses MockMvc, which never leaves the JVM. Added a `CorsConfigurationSource` scoped to `/api/**`, allowing exactly `grilld.frontend.base-url`'s origin. This is genuinely new attack surface (any origin allowed here can read authenticated responses in a browser), so it's intentionally narrow - one configured origin, not a wildcard, and only the methods/headers the frontend actually sends.

**`SessionStartResult`/`TurnAnswerResult` were missing `inputMode`/`whyAsking`** - both real fields on `InterrogatorTurnResult.NextQuestion` since Phase 4, persisted partially (`inputMode` lands on `Turn`) and dropped entirely otherwise (`whyAsking` was never persisted anywhere, not even a column for it). The frontend's chip/free-text hybrid input (`product-and-architecture.md` §2.2) can't render correctly without knowing which control to show, and the "why am I being asked this" one-liner is a real, already-written piece of curated copy that was simply never surfaced. Fixed by passing both straight through the live response - `whyAsking` isn't persisted to `Turn` (no column exists, and nothing needs to re-fetch a past turn's question yet), so this is intentionally the cheapest fix that's still honest, not a shortcut hiding a bigger gap.

**No endpoint existed to read a session back at all** - every prior phase only ever wrote to `discovery_sessions`/`slots`/`project_briefs`, never read them back through the API. The live brief panel (`product-and-architecture.md` §2.2's stated "magic-moment UX") is impossible to build without one. Added `GET /api/v1/sessions/{sessionId}` (`SessionService.getSessionDetail`) returning the raw idea, status, `brief_json`, scale tier, and every slot with its current status/value - enough for the frontend to render "X of Y things figured out" without inventing a second, frontend-side notion of the brief's state. Ownership-checked the same way Phase 8 checked everything else.

## Phase 10: production hardening, done before the rest of the frontend

Asked mid-Phase-9 to switch to backend production-readiness instead of continuing the frontend build - "what's actually left to go live" turned out to be a real, separate list from "what's left in the frontend spec": a persisted JWT key, per-user rate limiting, real object storage, and a Dockerfile for either service, none of which existed. Phase 10 closes exactly those four gaps and nothing else - see `docs/phases/phase-10/README.md` for the full architecture.

**The JWT signing key bug had been sitting there, documented, since Phase 1's own Javadoc** ("must be replaced with a persisted key before production use"). Fixing it needed a decision: fail fast on a missing key (matching `DB_URL`/`GOOGLE_CLIENT_ID`'s treatment) or keep the graceful ephemeral fallback (matching Lemon Squeezy's)? Chose graceful - unlike a missing DB or OAuth config, a missing signing key doesn't break anything at boot, it just silently keeps the exact behavior every test and every local dev session already depends on. Failing fast here would have meant threading a new required env var through 68 tests' worth of Spring context boot for a property none of them care about. `JwtKeyGenerator` (a `main()`-only class, deliberately not a Spring bean - it must never run as part of the app itself) is the one-off operational step that produces the value; `JwtConfigTest.restartingWithTheSamePersistedKeyAcceptsTokensIssuedBeforeTheRestart` is the test that actually proves the bug is fixed, not just that a JWK can be parsed.

**Rate limiting almost went in as a `bucket4j-spring-boot-starter`, then didn't.** The starter's declarative, properties-driven config is built around static per-path limits; what this needed was a limit keyed by the authenticated user (JWT subject), which the starter doesn't have a clean way to express. Went with `bucket4j-core` directly instead, in a hand-written `HandlerInterceptor` - more code, but the key extraction logic (subject-or-IP) is exactly the part that needed to be explicit anyway. Learned mid-implementation that `Bandwidth.classic(capacity, Refill)`/`Refill.intervally` - the API every current blog post and tutorial shows - is deprecated in the actual 8.10.1 release on Maven Central in favor of `Bandwidth.builder().capacity(n).refillIntervally(n, duration).build()`; caught by `-Dmaven.compiler.showDeprecation=true`, not by any doc, since the newest published docs/tutorials hadn't caught up to the library's own already-shipped deprecation.

**Rate limiting also nearly broke the existing suite before it shipped.** `SessionFlowIntegrationTest` and friends use MockMvc, which runs the full `HandlerInterceptor` chain - a real risk that a full interview loop (many `/answer` calls in one test method) would trip a default 20/minute limit and turn a correctness test into a flaky rate-limit test. Caught this before it happened (not after a red build): disabled rate limiting suite-wide via a surefire `systemPropertyVariables` entry, and wrote `RateLimitMvcIntegrationTest` as the one test that deliberately re-enables it via `@TestPropertySource` with a tiny capacity, to still prove the real MVC wiring rather than only the interceptor's logic in isolation.

**LocalStack, the obvious first choice for testing S3 storage, turned out not to be usable here.** As of March 2026 the `localstack/localstack` image requires a `LOCALSTACK_AUTH_TOKEN` from a LocalStack account just to start - a real, current change the Research-First check caught before writing a test against it. Testcontainers' dedicated MinIO module (`org.testcontainers:minio`, same version as the project's existing `postgresql`/`junit-jupiter` Testcontainers modules) needs no account and speaks the real S3 API closely enough that `S3PackageStorageTest` genuinely exercises `S3Client`'s request/response shapes, not a hand-rolled fake.

**The AI service's Dockerfile is generated, not written**, and that was a deliberate choice, not laziness: `grilld_ai_service/app.py`'s `get_graph()` factory already has a docstring explaining it's shaped to the exact signature the LangGraph Platform server requires to call it. That's a strong signal the service was already built to run behind `langchain/langgraph-api`, not behind a hand-rolled FastAPI wrapper - so `langgraph dockerfile --add-docker-compose` (the `langgraph-cli` skill's own documented command) produces the actually-correct packaging, and hand-writing an alternative would have fought the code's own existing shape. The one manual edit on top of the generated compose file: an explicit `DATABASE_URL` pointing at the *shared* grilld Postgres, since the generated stack's own bundled Postgres is only the platform server's internal run/task state, a different database serving a different purpose than what `get_graph()`'s checkpointer actually persists into.

**Flagged, not faked: running the AI service container needs a `LANGSMITH_API_KEY`.** The `langchain/langgraph-api` image checks for a license/API key at startup even in self-hosted "Lite" mode - confirmed via the LangChain forum and GitHub issues before assuming otherwise, since this is exactly the kind of "requires an account that doesn't exist in this environment" fact that's easy to get wrong by guessing. `docker build` for that image was still verified for real (it succeeds, dependencies resolve correctly inside the container); `docker compose up` was not attempted, since it would only fail predictably without that key.

**The backend Docker image was verified as a running container, not just a successful build** - a distinction worth being explicit about, since "docker build succeeded" proves far less than it sounds like. Ran the built image against the real local `grilld-postgres`, watched it apply/validate Flyway migrations and start Tomcat in the container logs, confirmed `/actuator/health` returned a real 200, and confirmed an unauthenticated API call still correctly got a 401 - proof the JAR, the JDK inside the image, and the full Spring Security filter chain all work identically to the host-run version, not just that `javac` didn't error.

## Phase 11: interview experience repair

Real usage (a captured chat transcript, `docs/chat.txt`) surfaced a batch of problems no amount of unit testing had caught, because they're all about the *feel* of a live conversation, not the correctness of any single API call: the Interrogator felt like an interrogation rather than a copilot, chip questions silently accepted more than one selection even though the backend contract requires exactly one, generation progress never visibly moved, and there was no way to fix a previous answer without restarting.

Fixed on both sides of the HTTP boundary at once, because the underlying cause was split across them: the Python Interrogator's own prompt (`interrogator/graph.py`) now explicitly recommends defaults instead of interrogating every uncertain answer, and treats delegation language ("decide for me") and stop language ("that's enough") as first-class conclusion triggers checked *before* another model call - so a clear "finish it" doesn't cost a full LLM round trip to honor. On the Spring side, `SessionService` gained real persistence for waived slots (a skip used to just move on; now it's recorded) and a fast-path finish detection that mirrors the Python side's own trigger phrases. On the frontend, chip questions became genuinely single-choice (`AnswerForm.tsx`), submitted controls lock instead of staying clickable, an always-visible "finish" action exists independent of whatever question is currently showing, `PUT /api/v1/sessions/{id}/slots/{slotKey}` makes the live brief panel's values actually editable, and `RunReportUpdate` gained real `completedAgents`/`totalAgents` counts so the progress bar has something honest to show instead of staying frozen.

No schema migration was needed - every fix here is either a prompt change, new application logic over existing columns, or frontend state handling.

## Product polish: profiles, session history, paid-only credits, a real landing page, a logo

A separate round of live-page feedback (empty-feeling landing page, profile showing an email instead of a real name, no visible way back to an older conversation, uncertainty about the "Grilld" name, wanting the pricing section to feel weightier since generation genuinely costs real tokens) drove a batch of changes that - unlike every phase before and after it - wasn't run through the phased-delivery checklist at the time, so it has no `docs/phases/phase-N/` folder of its own. Recorded here for completeness, not to model going forward.

**Real Google profile data.** `OAuth2LoginSuccessHandler` already had the OAuth2 user's `name`/`picture` attributes on hand and simply wasn't reading them. `User` gained `name`/`pictureUrl` columns (`V8__user_profile.sql`), `UserService.findOrCreateFromGoogle`'s signature grew two parameters, and the update now happens on *every* login (not just account creation) - a user who changes their Google profile photo sees it reflected on their next Grilld login, not frozen at signup time.

**Free credits removed entirely.** `users.credits_balance`'s default changed from 60 to 0 - every credit is now purchased, no exceptions, matching a deliberate product decision that generation is expensive enough (real Anthropic token spend per run) that a free grant isn't sustainable. This one change had a wide blast radius: every test that exercises credit-spending logic needed a `creditService.grantIdempotent(user.getId(), 60, "test-setup-grant:" + googleId)` added right after creating its test user, and `LemonSqueezyWebhookControllerTest`'s literal expected balances all shifted down by 60. A mechanical but wide-reaching change, not a risky one - `CreditService` remains the only writer of the balance either way.

**Session history and resume.** Nothing before this point ever read `discovery_sessions` back as a *list* - every prior read was "give me this one session I already know the id of." `SessionService.listSessions`/`getTurnHistory` plus two new endpoints (`GET /sessions`, `GET /sessions/{id}/turns`) make "show me my past conversations" possible at all. The frontend's interview landing screen grew a clickable list of past sessions; picking one rebuilds the chat transcript from the turn history. (This resume was incomplete until Phase 12 closed the other half - see below - it never checked whether the session already had a finished blueprint.)

**Login persistence, actually.** `TokenService`'s JWT expiry moved from 24 hours to 30 days, and the frontend's own auth cookie `maxAge` matches it. Worth knowing (see Phase 10's own section above): this only holds if `grilld.jwt.signing-key-jwk` is set to a persisted key in production - the ephemeral fallback invalidates every token on every restart regardless of the expiry configured on the token itself, since the key used to verify it no longer exists.

**A real logo.** A flame mark (matching the "grilled" idea already baked into the brand and copy) as an SVG favicon and the site's wordmark everywhere it appears - see `grilld-frontend/src/components/Logo.tsx`.

**A genuinely redesigned landing page**, including an agent-pipeline diagram built with `@xyflow/react` (React Flow) - a static, hand-positioned node/edge diagram (no layout engine needed, since the content's shape never changes) showing idea → interrogation → orchestrator → the ten specialists → output, matching `RunReportService`'s own agent roster exactly so the marketing page and the real product never drift apart. See `grilld-frontend/LEARNING.md` for the Tailwind theming details underneath it.

## Phase 12: voice input plumbing, rendered diagrams, and run history restore

Built from a fresh full-project audit (not a bug report) - see `docs/phases/phase-12/README.md` for the complete write-up; this section covers the backend-side "why," matching this file's usual depth.

**The `TranscriptionService` seam.** The user knew *that* they wanted voice input, and knew it should eventually call a real speech-to-text provider (ElevenLabs Scribe, Fish Speech, or similar), but hadn't picked one yet - "build everything around it" was the actual instruction. This is exactly the situation `AiServiceClient` and `PackageStorage` were already built to handle: define the interface and the caller-side plumbing now, ship one honestly-failing default implementation, and let the real implementation be a self-contained follow-up with nothing above it needing to change. `UnconfiguredTranscriptionService` (active whenever `grilld.voice.provider=none`, today's only value) throws a new `TranscriptionUnavailableException` - mapped by `GlobalExceptionHandler` to a 503 with a message the frontend shows verbatim, not a silent no-op or a fabricated transcript.

**Reusing the interview's own rate-limit tier.** `POST /api/v1/voice/transcribe` isn't its own new cost tier in `RateLimitConfig` - it's added to the existing "interview" bucket, since recording a voice answer and typing one are the same per-turn action from a cost/abuse perspective, not two different things needing two different limits.

**`GET /sessions/{id}/runs` closes a real gap the session-history feature (above) left open.** Reopening a session whose blueprint had already generated showed an empty `GenerationPanel`, because nothing existed to answer "does this session already have a run?" `GenerationService.listRuns` reuses the exact same ownership-check pattern `generate()` itself already established (Phase 7/8) and the already-existing `findByBriefIdOrderByStartedAtDesc` repository method - genuinely small, because the hard infrastructure (ownership checks, the run/report/package read endpoints) already existed from earlier phases; only the "list them" read was missing.

**A document-preview endpoint fell out of building the above.** Once a restored run's Run Report was reachable again, showing its per-document list without any way to actually read one felt like a dead end - `GET /sessions/{id}/runs/{runId}/documents?path=...` reads a single `GeneratedDocument`'s persisted content directly (the same table `PackagerService` already zips from), so the frontend can preview a file before committing to a full zip download. Small and additive: no new persistence, just a new read path over data that was already being stored since Phase 6.

**Testing note:** this session's sandbox had Docker Desktop installed but its daemon not running, which blocks every Testcontainers-backed test (`GenerationServiceTest` and friends all boot a real Postgres container per test class). Started Docker Desktop mid-session and re-ran the full suite once it was up - all 73 tests (68 pre-existing + 5 new) passed clean. Worth remembering for next time: a `docker ps` failing with "cannot find the pipe" almost always means the daemon isn't running yet, not that Docker isn't installed - the CLI and the daemon are separate things to check.

### Fish Audio wired in as the real provider

Once the user picked Fish Audio, per the Research-First Rule the actual contract was fetched live rather than assumed - "Fish Speech"/"Fish Audio" familiarity from training data would have been a real risk here anyway, since the endpoint used (`POST https://api.fish.audio/v1/asr`) is explicitly marked BETA on Fish Audio's own OpenAPI reference, meaning it's exactly the kind of thing that can change shape between when a model was trained and now. Fetched `docs.fish.audio/api-reference/endpoint/openapi-v1/speech-to-text` directly and confirmed the exact contract before writing any code: Bearer auth, `multipart/form-data` only (their own docs explicitly call out that base64-in-JSON is *not* supported - a real gotcha, since that's a common enough pattern elsewhere that assuming it would work here would've been an easy, wrong guess), one required `audio` field, and a JSON response whose only field this use case needs is `text`.

`FishAudioTranscriptionService` slots into the seam exactly like the plan called for - `@ConditionalOnProperty(prefix = "grilld.voice", name = "provider", havingValue = "fishaudio")`, nothing above it (the controller, the frontend) changed at all. One small deliberate departure from `HttpAiServiceClient`'s existing pattern of calling the static `RestClient.create(baseUrl)`: this one takes an injected `RestClient.Builder` instead (Spring Boot auto-configures one) purely so `FishAudioTranscriptionServiceTest` can substitute a `MockRestServiceServer`-bound builder and assert on the *real* request produced - the Bearer header, the multipart Content-Type, the actual URL - without a live network call. `HttpAiServiceClientTest` had already established this codebase's own answer to "how do you test code that calls a real external HTTP API": extract the parsing logic and test that against a captured fixture, not the network call itself. `MockRestServiceServer` is the more direct tool for a one-shot request/response call like this one (no complex streaming to extract), and it wasn't available as an option for `HttpAiServiceClient` at the time that pattern was set, but it's a better fit here - genuinely testing the request-building code, not sidestepping it.

The one thing this service can't get right from documentation alone: what audio format the multipart part's filename/Content-Type should carry. The docs never say which formats are accepted, only that JSON/base64 isn't. Handled by passing the browser's own real `Content-Type` (`audio/webm`, from `MediaRecorder`) straight through as the part's header, with a best-effort filename extension derived from it - honest about being a guess in the code comment, not asserted as a documented requirement it isn't.

**Honest gap, not faked:** the Interrogator's structured output carries an `inputMode` of `chips` as a real, spec'd possibility, but never returns concrete chip *option values* - nothing in `InterrogatorTurnResult.NextQuestion` carries them, and `StubAiServiceClient` has only ever exercised `text`/`number`. The frontend (`AnswerForm.tsx`) falls back to a text control for `chips` and `voice_primary` rather than inventing plausible-looking chip labels or a fake microphone button with no speech-to-text behind it. Real chip rendering needs the Interrogator to actually return option values first - that's Python-side, not a frontend gap to paper over.
