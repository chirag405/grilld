...
- **Doc curation and arbitrary user requests during generation — fixed API parameters, or an intelligent router?** Users will ask for things no fixed flag list anticipates: "skip the market docs," "add a doc for X," "I need this fast," "create a whole new doc type for this one feature." Raised directly (Phase 5 retro, 2026-08-16) with a strong instinct worth taking seriously: rather than Spring pre-encoding every possible variation as an API parameter, the Orchestrator itself (already an LLM, already the thing delegating to specialists) could interpret open-ended natural-language requests and decide how to route them - closer to how Claude Code's own agent works than to a rigid settings panel. Checked against current (2026) multi-agent orchestration research before writing this down: this is a real, named production pattern - "Router / Dynamic Handoff" - one of five dominant 2026 orchestration topologies, and Claude Code itself is moving the same direction ("dynamic workflows," research preview). It's a legitimate direction, not just a hunch. It also comes with documented production failure modes worth designing around deliberately rather than discovering the hard way: **infinite handoff loops** (routing in circles because no agent/step "owns" the task), **context degradation** across handoffs (passing full context gets expensive/hits token limits; summarizing at each hop compounds errors), and **non-deterministic routing** (the same request can produce different agent chains run to run, which makes both debugging and cost prediction harder) - none of which have an off-the-shelf fix in the sources checked, so Grilld's own guardrails (loop detection, a hop budget, some bound on how many ad-hoc "doc agents" one request can spin up) would need to be designed here, not assumed to come free with the pattern. Also open: how this interacts with the fixed 10-agent roster (§3.2) and the scale-tier ceiling. Explicitly deferred to be planned as its own phase, not bolted onto Phase 5's fixed-roster implementation after the fact.
